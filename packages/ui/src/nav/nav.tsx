...
import { APP_DOMAIN, cn, createHref, fetcher } from '@repo/utils';
...
export function Nav({
  ...
  let { domain = 'trn.com' } = useParams() as { domain: string };
  if (staticDomain) {
    domain = staticDomain;
  }
  ...
  const { data: session, isLoading } = useSWR(
    domain.endsWith('trn.com') && '/api/auth/session',
    fetcher,
    {
              ...
              >
                    ...
                    ({ name, href, segments, content: Content }) => {
                      ...
                      return (
                        <NavigationMenuPrimitive.Item key={name}>
                          ...
                          {Content && (
                            <NavigationMenuPrimitive.Content className="radix-motion-from-start:animate-enter-from-left radix-motion-from-end:animate-enter-from-right radix-motion-to-start:animate-exit-to-left radix-motion-to-end:animate-exit-to-right absolute left-0 top-0">
                ...
                <div className="absolute left-1/2 top-full mt-3 -translate-x-1/2">
                  <NavigationMenuPrimitive.Viewport
                    className={cn(
                      'relative flex origin-[top_center] justify-start overflow-hidden rounded-[20px] border border-neutral-200 bg-white shadow-md dark:border-white/15 dark:bg-black',
                      'radix-state-closed:animate-scale-out-content radix-state-open:animate-scale-in-content',
                      'h-radix-navigation-menu-viewport w-radix-navigation-menu-viewport transition-[width,height]'
                    )}
                  />
                </div>
              </NavigationMenuPrimitive.Root>
              ...
              <div className="hidden grow basis-0 justify-end gap-2 lg:flex">
                {session && Object.keys(session).length > 0 ? (
                  ...
                  <>
                    ...
                    <Link
                      href="https://app.trn.com/register"
                      className={cn(
                        buttonVariants({ variant: 'primary' }),
                        'flex h-8 items-center rounded-lg border px-4 text-sm',
                        'dark:border-white dark:bg-white dark:text-black dark:hover:bg-neutral-50 dark:hover:ring-white/10'
  ...
  return (
    ...
    >
      <path
        ...
        className="transition-transform duration-150 transform-view origin-center [vector-effect:non-scaling-stroke] group-radix-state-open/item:-scale-y-100"
      />
    </svg>
  );
}
