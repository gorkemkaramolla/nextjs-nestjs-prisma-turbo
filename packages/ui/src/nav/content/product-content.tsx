import { cn, createHref } from '@repo/utils';
...
const largeLinks = [
  {
    ...
    href: '/integrations',
    graphic: (
      <div className="absolute -right-4 top-1/2 h-[180px] w-[240px] -translate-y-1/2 mask-[linear-gradient(90deg,black_50%,transparent_95%)] dark:opacity-80">
        <Image src="/assets/images/integrations-grid.png" alt="" fill />
      </div>
    ),
  },
  {
    ...
    graphic: (
      <div className="absolute -right-4 top-2.5 h-[180px] w-[240px] mask-[linear-gradient(90deg,black_50%,transparent_95%)] dark:opacity-60">
        <Image src="/assets/images/api-thumbnail.png" alt="" fill />
      </div>
    ),
  },
];
        ...
        {products.map(
          ({
            ...
            <NavigationMenuLink asChild key={title}>
              ...
              >
                <Grid
                  className="mask-[linear-gradient(transparent,black,transparent)] dark:text-white/5"
                  cellSize={60}
                  patternOffset={[-51, -23]}
                />
                ...
                <div className="relative mt-10 h-40 grow">
                  <div
                    className={cn(
                      'absolute left-0 top-0 size-full grow overflow-hidden mask-[linear-gradient(black_50%,transparent)]',
                      graphicsContainerClassName
                    )}
                  >
      ...
      <div className="grid grow grid-cols-2 gap-4">
        {largeLinks.map(({ title, description, href, graphic }) => (
          <NavigationMenuLink asChild key={title}>
            ...
            >
              <Grid
                className="mask-[linear-gradient(90deg,transparent,black)] dark:text-white/5"
                cellSize={60}
                patternOffset={[-39, -49]}
              />
              <div
