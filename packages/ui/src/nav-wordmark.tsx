...
export function NavWordmark({
  ...
  const { domain = 'dijimall.com' } = useParams() as { domain: string };
  ...
  return (
    <Popover.Root open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
      ...
      <Popover.Portal>
        ...
        >
          <div className="grid gap-1 rounded-lg border border-neutral-200 bg-white p-2 drop-shadow-sm sm:min-w-[240px] dark:border-white/15 dark:bg-black">
            <ContextMenuButton
            ...
            <ContextMenuButton
              ...
              variant="outline"
              onClick={() =>
                window.open('https://dijimall.com/brand', '_blank')
              }
              icon={<BoxSelect strokeWidth={2} className="h-4 w-4" />}
            />
            {/* If it's in the app or it's a domain placeholder page (not dijimall.com homepage), show the home button */}
            {isInApp || domain != 'dijimall.com' ? (
              <ContextMenuButton
                text="Home Page"
                variant="outline"
                onClick={() =>
                  window.open(
                    `https://dijimall.com${isInApp ? '/home' : ''}`,
                    '_blank'
                  )
                }
              ...
              <ContextMenuButton
                ...
                variant="outline"
                onClick={() =>
                  window.open('https://app.dijimall.com', '_blank')
                }
...
function ContextMenuButton({ className, ...rest }: ButtonProps) {
  return (
    <Button
      className={cn(
        'h-9 justify-start px-3 font-medium hover:text-neutral-700 dark:text-white/70 dark:hover:bg-white/15 dark:hover:text-white',
        className
      )}
      {...rest}
    />
