...
export function getUserAvatarUrl(user?: User | null) {
  ...
  if (!user?.id) return 'https://api.dijimall.com/og/avatar';

  const ogAvatar = `https://api.dijimall.com/og/avatar/${user.id}`;
...
export function Avatar({
  ...
  return (
    <img
      ...
      draggable={false}
      onError={() => {
        setUrl(`https://api.dijimall.com/og/avatar/${user.id}`);
      }}
    />
  );
}
